...
> ⚠️ Этот файл — старая dev-шпаргалка и местами устарел (модель «пациент по ссылке без регистрации», `PatientView.js`, порт 5000 — неактуальны). Источник правды по продукту — `CLAUDE.md`. Источник UI-подписей — реальные компоненты `frontend/src/pages/...`.

## 📘 Пользовательский гайд `docs/guide/` — держать в актуальном состоянии

В репозитории есть конечно-пользовательское руководство `docs/guide/` (README + 01–15, два трека: пациент и оператор). ТЗ — `TZ_USER_GUIDE_2026-06-04.md`.

**Правило:** при заметной пользовательской фиче раздел гайда обновляется **в том же PR**:
- источник UI-подписей — **компонент** `frontend/src/pages/...`, не память и не dev-шпаргалки;
- обнови дату в футере (`_Обновлено: …_`) затронутых файлов;
- бамп Service Worker (`SW vN`) без правки `docs/guide/` — повод проверить гайд;
- в `SESSION_HANDOFF` добавляй строку `Guide: обновлён раздел NN` / `Guide: не требуется`.

См. §9 ТЗ — механизм поддержания актуальности (PR-template `.github/pull_request_template.md` уже добавлен).
