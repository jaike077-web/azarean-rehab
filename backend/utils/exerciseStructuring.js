...
function buildConsistencyPrompt(fields) {
  const system = [
    'Ты — редактор библиотеки упражнений Azarean. Проверь карточку на ЕДИНООБРАЗИЕ и стиль',
    '(не клинику, не полноту). Отметь нарушения, например:',
    '   - обращение к пациенту НЕ на «вы» («ты»/инфинитив) в пациент-видимых полях',
    '     (description, instructions, cues, tips);',
    '   - сложные анатомические термины в description (там — простой язык; точные термины',
    '     уместны в instructions/cues);',
    '   - разнобой терминологии внутри карточки (одно и то же названо по-разному);',
    '   - тон/регистр скачет между полями.',
    '',
    'СТРОГО:',
    '1. Только ЯВНЫЕ нарушения единообразия. Стилевые придирки на вкус — пропускай.',
    '2. НЕ переписывай поля — указывай, ЧТО привести к единому виду.',
    '3. Если всё единообразно — верни {"concerns": []}.',
    '4. Ответ — ТОЛЬКО валидный JSON (без markdown):',
    '   {"concerns":[{"severity":"high|medium|low","field":"имя поля","message":"что унифицировать"}]}',
  ].join('\n');
  const user = ['КАРТОЧКА УПРАЖНЕНИЯ (JSON):', JSON.stringify(fields || {}, null, 0)].join('\n');
  return { system, user };
}
