...
function buildScriptPlannerPrompt(input) {
  ...
  if (i.notes) parts.push(`ЧЕРНОВИК/заметки инструктора (опирайся и расширь, не отбрасывай): ${i.notes}`);
  ...
  const system = [
    ...
    '',
    'СТРОГО:',
    '1. Это ЧЕРНОВИК ПОД ВЫЧИТКУ ЭКСПЕРТОМ, но script читается как ЧИСТЫЙ ПАЦИЕНТСКИЙ ТЕКСТ.',
    '   Где предлагаешь КЛИНИЧЕСКОЕ значение (угол, длительность удержания, противопоказание,',
    '   уместность фазы, нагрузка) — поставь в тексте script КОНКРЕТНЫЙ плейсхолдер-значение',
    '   («наклон примерно 30–40°»), как готовую формулировку. А факт «это надо подтвердить»',
    '   вынеси ОТДЕЛЬНО в review_points. В САМОМ тексте script НЕ пиши «уточните…» / «уточни',
    '   у инструктора» / «(уточните диапазон)» — эти ремарки живут ТОЛЬКО в review_points,',
    '   иначе они протекут в пациентские поля при разборе и испортят faithfulness-оценку.',
    '   Итого: script = чистый текст с конкретными значениями; review_points = чек-лист эксперта.',
    ...
    '   Лучше явный запрос «уточните …» в review_points, чем уверенная выдумка.',
    '3a. Если дан ЧЕРНОВИК/заметки инструктора — это его опорный текст: СОХРАНИ всю названную',
    '   им специфику (значения, акценты, формулировки), не противоречь и не выкидывай. Расширь',
    '   его до ПОЛНОГО скрипта по чек-листу, дозаполнив недостающее. Спорную клинику из черновика',
    '   не переписывай молча — вынеси на подтверждение в review_points.',
    '4. СНАЧАЛА пойми, КАКОЕ это движение. В начале description одной фразой назови, как ты',
    '   понял упражнение («Понял как: …»). Если название НЕОДНОЗНАЧНО или ты не уверен, что',
    '   за движение (редкое/нестандартное/студийное) — НЕ угадывай уверенно: построй скрипт',
    '   по наиболее вероятному прочтению И поставь ПЕРВЫМ пунктом review_points',
    '   «Название неоднозначно — подтвердите: имелось в виду X, а не Y». Чем меньше уверенность,',
    '   тем КОРОЧЕ и осторожнее скрипт (меньше выдуманных деталей), больше пунктов на подтверждение.',
    '5. ДОЗИРОВКИ держи арифметически согласованными: если расписываешь подходы/циклы/секунды,',
    '   итоговая длительность обязана сходиться с суммой частей. Проверь сам: НЕ пиши «2–3 минуты»,',
    '   если 3 цикла × 3 движения × 30 сек = 4.5 мин. Либо поправь числа, либо общую длительность.',
    '6. Ответ — ТОЛЬКО валидный JSON (без markdown):',
...
// ──────────────────────────────────────────────────────────────────────────
// КРИТИК ПОЛНОТЫ (агент №5). ОТДЕЛЬНО от sanity (безопасность) и faithfulness
// (верность источнику): проверяет ЦЕЛОСТНОСТЬ описания для пациента — на месте ли
// обязательные слоты под тип движения. Только указывает «чего не хватает», полей
// не меняет, значений не выдумывает. Выход — тот же {concerns:[...]} → summarizeSanity.
// ──────────────────────────────────────────────────────────────────────────

function buildCompletenessPrompt(fields) {
  const system = [
    'Ты — методист-реабилитолог. Проверь карточку упражнения на ПОЛНОТУ — не безопасность',
    'и не верность источнику, а целостность описания для пациента. По типу движения отметь,',
    'чего НЕ ХВАТАЕТ для корректного и безопасного выполнения, например:',
    '   - не указана стартовая позиция / постановка опорных точек (стопы, опора руки);',
    '   - удержание/изометрия без длительности; подходы/повторы не названы там, где это важно;',
    '   - для удерживаемого/нагрузочного движения не указано дыхание;',
    '   - нет амплитуды/угла там, где они критичны для техники;',
    '   - в прогрессии не указано НАПРАВЛЕНИЕ (как именно усложнять);',
    '   - нет противопоказаний для явно рискового движения.',
    '',
    'СТРОГО:',
    '1. Отмечай только СОДЕРЖАТЕЛЬНЫЕ пробелы, влияющие на выполнение. Мелочи — пропускай.',
    '2. НЕ переписывай упражнение, НЕ выдумывай значения — только указывай, ЧЕГО не хватает.',
    '3. Если для этого типа всё полно — верни {"concerns": []}.',
    '4. Ответ — ТОЛЬКО валидный JSON (без markdown):',
    '   {"concerns":[{"severity":"high|medium|low","field":"имя поля","message":"чего не хватает"}]}',
  ].join('\n');
  const user = ['КАРТОЧКА УПРАЖНЕНИЯ (JSON):', JSON.stringify(fields || {}, null, 0)].join('\n');
  return { system, user };
}
...
// ──────────────────────────────────────────────────────────────────────────
// КРИТИК КОНСИСТЕНТНОСТИ (агент №6). Единообразие/стиль (не клиника, не полнота):
// форма обращения «вы», простой язык в description vs термины в instructions/cues,
// единая терминология. Только указывает, полей не меняет. Выход {concerns:[...]}.
// ──────────────────────────────────────────────────────────────────────────

function buildConsistencyPrompt(fields) {
  const system = [
    'Ты — редактор библиотеки упражнений Azarean. Проверь карточку на СТИЛЬ, обращение и',
    'читаемость (не клинику, не полноту). Отметь нарушения, например:',
    '   - обращение к пациенту НЕ на «вы» («ты»/инфинитив) в пациент-видимых полях',
    '     (description, instructions, cues, tips);',
    '   - в description сложные термины БЕЗ простого пояснения (description — для пациента,',
    '     простым языком; точные термины уместны в instructions/cues);',
    '   - тон/регистр резко скачет между полями (где-то сухо-клинически, где-то разговорно).',
    '',
    'ВАЖНО про СИНОНИМЫ: вариативность терминов УМЕСТНА и ЖЕЛАТЕЛЬНА. «Квадрицепс»,',
    '«четырёхглавая мышца бедра», «передняя поверхность бедра» — каждый к месту (точный',
    'термин в инструкциях, простой — в description). Это ЖИВОЙ язык, а НЕ нарушение — НЕ флагуй',
    'разные синонимы как «разнобой». Термин отмечай ТОЛЬКО если он реально вводит пациента',
    'в заблуждение (читается как ДРУГОЙ объект/мышца/движение).',
    '',
    'СТРОГО:',
    '1. Только ЯВНЫЕ проблемы обращения/стиля. Придирки на вкус и СИНОНИМЫ — пропускай.',
    '2. НЕ переписывай поля — указывай, ЧТО поправить.',
    '3. Если всё ок — верни {"concerns": []}.',
    '4. Ответ — ТОЛЬКО валидный JSON (без markdown):',
    '   {"concerns":[{"severity":"high|medium|low","field":"имя поля","message":"что поправить"}]}',
  ].join('\n');
  const user = ['КАРТОЧКА УПРАЖНЕНИЯ (JSON):', JSON.stringify(fields || {}, null, 0)].join('\n');
  return { system, user };
}
