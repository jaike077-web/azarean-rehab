-- ACL default phase transition criteria seed (dev convenience / clinical review)
-- АВТОРИТЕТНАЯ копия — backend/database/migrations/20260518_acl_criteria_seed.sql
-- При расхождениях миграция — источник истины.
--
-- ~30 критериев распределены по 6 фазам ПКС (Защита → Полное восстановление).
-- Источник: AAOS/APTA guidelines (Claude initial draft).
-- Vadim ревьюит и калибрует через AdminContent → Фазы → "Критерии ▼" после деплоя.
-- Идемпотентно: ON CONFLICT (phase_id, criterion_code) DO NOTHING.
