// =====================================================
// TESTS: ExerciseRunner — CP3a.1 per-set countdown + auto-rest
//
// Узкий unlock LOCKED-раннера (auto_complete=true, duration_seconds>0).
// CP3a.2 (auto_complete=false, open stopwatch) — отдельный TZ.
//
// Покрытие:
//  - 3 sets timed: 3 countdown'а + 2 авто-rest'а; cue('set_end') ×3.
//  - Ручной «Раньше»: completeSet без ожидания 0, без cue('set_end').
//  - Anti-regression: POST /api/progress (submit упражнения) — toHaveBeenCalledTimes(1).
//  - Anti-regression rep-only: гайд НЕ активируется, прежний flow 1:1.
//  - Anti-regression auto_complete=false: гайд НЕ активируется (CP3a.2 scope).
//
// Mock-стратегия:
...
describe('CP3a.1 — per-set countdown + auto-rest (auto_complete=true)', () => {
  ...
  it('Anti-regression auto_complete=false: гайд НЕ активен — открытый секундомер сохранён (CP3a.2 scope)', () => {
    renderRunner({ auto_complete: false });
    // Гайд НЕ активен
    expect(screen.queryByTestId('set-indicator')).not.toBeInTheDocument();
    expect(screen.queryByTestId('set-countdown')).not.toBeInTheDocument();
    expect(screen.queryByTestId('finish-set-early-btn')).not.toBeInTheDocument();
    // Существующий открытый секундомер (count-UP) — рендерится
