...
const Toast = ({ id, type = 'info', title, message, duration = 4000, onClose }) => {
  ...
  return (
    <div className={`${s.toast} ${s[`toast${type.charAt(0).toUpperCase()}${type.slice(1)}`] || ''} ${isExiting ? s.toastExiting : ''}`} role="status" aria-live="polite">
      <span className={s.toastIcon}>
        <Icon size={20} aria-hidden="true" />
      </span>

      <div className={s.toastContent}>
        <p className={s.toastTitle}>{title || TITLES[type]}</p>
        {message && <p className={s.toastMessage}>{message}</p>}
      </div>
