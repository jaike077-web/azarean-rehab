...
## Smoke-test чеклист (после первого успешного деплоя)

Выполнить в **инкогнито-окне** браузера (чтобы dev-cookies не мешали). Архитектор согласовал порядок 2026-04-23:

```
1. Открыть https://my.azarean.ru/
   ✓ Страница загружается, TLS-замок зелёный
   ✓ DevTools → Security → сертификат Let's Encrypt, валиден

2. DevTools → Network → F5 (reload)
   ✓ Все запросы 200 (ни одного красного)

3. Логин пациента: avi707@mail.ru / Test1234
   ✓ Редирект на /patient-dashboard
   ✓ v12-интерфейс грузится (pulse-dot на Roadmap, PGIC на Home)
   ✓ DevTools → Application → Cookies:
     - patient_access_token:  HttpOnly=true, Secure=true, SameSite=Lax
     - patient_refresh_token: HttpOnly=true, Secure=true, SameSite=Lax

4. На Home тапнуть PGIC «Лучше»
   ✓ Открыть Diary — слайдер pre-populated на pain_level=2
   ✓ POST /api/rehab/my/diary → 200

5. Логаут → логин инструктора: vadim@azarean.com / Test1234
   ✓ Редирект на /dashboard (таб-контейнер)
   ✓ Статистика: пациенты/комплексы/упражнения числа (не —)

6. Открыть Pacients → карточка id=14
   ✓ GET /api/patients → 200
   ✓ БД-проверка: SELECT * FROM audit_logs ORDER BY created_at DESC LIMIT 3;
     должна быть свежая запись action='READ' entity_type='patient' entity_id=14
```

Если все 6 шагов прошли — deploy успешный.
