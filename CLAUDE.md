...
> ⏭️ **Отложено в M2 (отдельный заход):** (1) **связь зон** — инструктор-нота «как одна зона влияет на другую» (поле + админ-UI + показ вверху «Пути»); (2) **оркестровка дня/цикла** при ≥2 комплексах (экран «Сегодня»). **Хвосты TZ:** карточка-шапка при ОДНОЙ программе (сейчас голый таймлайн); цвет точки фазы из хелпера таймлайна. Hardening гонки `priority` при параллельном создании — в M3. Детали: [[project-multiprogram-path]], [SESSION_HANDOFF_2026-06-19_MULTIPROGRAM_LIVE.md](SESSION_HANDOFF_2026-06-19_MULTIPROGRAM_LIVE.md).
