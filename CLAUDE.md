...
| ~~12~~ | ~~MEDIUM~~ | ~~Hardcoded «ПКС» в HomeScreen hero~~ → **HomeScreen закрыт** Wave 0 commit 02 (`8c9df80`, ветка `wave-0/02-home-dynamic-label`, PR #46): `program.program_label` приходит из dashboard endpoint, regex-маппинг 8 категорий через `backend/utils/programLabels.js`. **Остатки для Волны 1:** `services/telegramBot.js:170` (`WHERE program_type = 'acl'`) и `RoadmapScreen.js` (`?type=acl` дефолт) — будут заменены на справочник `program_types` в миграции Волны 1. |
