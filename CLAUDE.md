...
### ✅ Wave 1 — ЗАКРЫТА в проде (2026-05-15)

Multi-protocol foundation + program_templates + 3-step wizard для RehabProgramModal + stuck-detection инструктора. **Все 10 коммитов смерджены в main строго в порядке** через cycle retarget → rebase → force-push → `gh pr merge --squash`.

| # | SHA на main | PR | Что | Bug |
|---|---|---|---|---|
| 1.01 | `f71038e` | [#51](https://github.com/jaike077-web/azarean-rehab/pull/51) | Миграция `program_types` + поле `rehab_programs.program_type` + FK | — |
| 1.02 | `9ba85a4` | [#52](https://github.com/jaike077-web/azarean-rehab/pull/52) | `GET /api/rehab/program-types` публичный + JOIN program_types в `/my/dashboard` | — |
| 1.03 | `a995ef1` | [#53](https://github.com/jaike077-web/azarean-rehab/pull/53) | Удалён `backend/utils/programLabels.js` regex-маппинг | — |
| 1.04 | `d9fab17` | [#54](https://github.com/jaike077-web/azarean-rehab/pull/54) | Динамический program_type (api.js + RoadmapScreen + telegramBot) | #12 ✅ |
| 1.05 | `b34ad63` | [#55](https://github.com/jaike077-web/azarean-rehab/pull/55) | AdminContent CRUD program_types + select PhaseForm + filter PhasesTab | — |
| 1.06 | `f6f9617` | [#56](https://github.com/jaike077-web/azarean-rehab/pull/56) | Миграция `program_templates` + junction + 2 endpoints + POST /programs принимает template_id | — |
| 1.07 | `16b300a` | [#57](https://github.com/jaike077-web/azarean-rehab/pull/57) | AdminContent ProgramTemplatesTab + 7 endpoints + PhaseComplexEditor | — |
| 1.08a | `81c4305` | [#58](https://github.com/jaike077-web/azarean-rehab/pull/58) | `derived_title` computed field в 4 SELECT'ах `routes/complexes.js` | #13 backend prep |
| 1.08b | `e7b4944` | [#59](https://github.com/jaike077-web/azarean-rehab/pull/59) | RehabProgramModal → директория (CreateWizard + EditForm + ComplexSelector) | #13 ✅ |
| 1.09 | `c74c468` | [#60](https://github.com/jaike077-web/azarean-rehab/pull/60) | Stuck detection инструктор (yellow badge + opsAlert red push + weekly cron Mon 09:00 МСК) | — |

**Метрики:** Backend 423/423 (+85), Frontend 252/252 (+16). 3 миграции идемпотентны.

**Prod-smoke 2026-05-15:** Сценарии 1-4 ✅ (ACL + CreateWizard + Shoulder label + AdminContent template). Сценарий 5 (red push) ⚠️ — покрыт 8 mock-based unit-тестами; плановый cron Mon 09:00 МСК подтвердит на проде.

**8 backlog findings из smoke** зафиксированы в memory: 4 hot-fix mini-PR (~30 мин каждый: AdminContent CSS, complexes.title UI, patient stuck-status acl, invite-code UX), 1 архитектор-fix (OAuth `password_hash IS NULL` — **блокер коммерческого запуска**), 2 архитектор-вопроса (seed phases для non-ACL, N программ на пациента).

**Dirty файлы dark-theme от 2026-05-04 нетронуты во всех 10 коммитах** — изоляция сохранена через `git stash` перед каждым rebase.

**Точка входа в новый чат:** [`SESSION_HANDOFF_2026-05-15_WAVE1_DONE.md`](SESSION_HANDOFF_2026-05-15_WAVE1_DONE.md). Memory: [`wave_1_complete.md`](.claude/projects/c--Users-------Desktop-Azarean-rehab/memory/wave_1_complete.md).
...
**Wave 1 hot-fix batch 2026-05-15 (PR #61-#66) — закрыт в проде:** 6 PR'ов в одной сессии после Wave 1 retrospective. #61 multi-protocol cleanup (Bug #12 polished), #62 OAuth post-registration (Bug #5 — блокер коммерч.запуска снят: phone-match без `password_hash IS NULL` + email-fallback Yandex, password_hash не затирается), #63 complex.title field UI (Bug #13 root cause — POST/PUT принимают title + UI inputs в Create/Edit с pre-fill), #64 AdminContent CSS Modules (22 класса скопированы из AdminUsers), #65 invite-code share UX (pre-filled `?code=` в URL + Copy-link button + PatientRegister auto-fill), #66 SW bump v3→v4 (cache invalidation для #65 — без этого Telegram-share показывал старый bundle). Backend 423→**437** (+14 тестов), frontend 252/252, 0 миграций. Prod-smoke 4/4 ✅. Memory: [`wave_1_hotfix_batch_complete.md`](.claude/projects/c--Users-------Desktop-Azarean-rehab/memory/wave_1_hotfix_batch_complete.md). Точка входа в новый чат: `SESSION_HANDOFF_2026-05-15_HOTFIXES_DONE.md`.

**Параллельная сессия 2026-05-16 (пока ждём Wave 2):** 2 backlog задачи в одном чате между Wave 1 и Wave 2. **#03 Wave 1 retrospective audit** — добавочный grep по `api.js`, `RoadmapScreen`, `telegramBot` (handoff backlog HIGH). Результат **CLEAN**: 0 функциональных хардкодов вне комментариев; 5 оставшихся `'acl'` в production-коде — by-design defaults. Bug #12 закрыт окончательно. Memory: [`wave_1_retrospective_audit_2026-05-16.md`](.claude/projects/c--Users-------Desktop-Azarean-rehab/memory/wave_1_retrospective_audit_2026-05-16.md). **#01 AdminContent dark inputs (PR #67, `16ed04c`)** — partial fix для Bug #15: `background: var(--color-surface-2)` в `.adminFormGroup input/select/textarea`. Bug #15 родительский (MDEditor + global inputs в `index.css` + `color-scheme: dark`) — остаётся открытым. **#02 Seed phases для shoulder_general/knee_general** — ТЗ-структура готова в [TZ_PARALLEL_02_SEED_PHASES_NON_ACL.md](TZ_PARALLEL_02_SEED_PHASES_NON_ACL.md), блокировано clinical content (количество фаз / названия / duration / цели / red flags / FAQ). Backend 437/437, frontend 252/252. **Wave 2 ждёт ответы Vadim'а архитектору на 5 OPEN questions (measurements + pain events).**
...
- **Wave 1 — все 10 PR смерджены в main (2026-05-15, stacked PR batch merge):** PR #51-#60 закрыты в main строго в порядке через cycle retarget→rebase→force-push→squash-merge. SHA на main: `f71038e → 9ba85a4 → a995ef1 → d9fab17 → b34ad63 → f6f9617 → 16b300a → 81c4305 → e7b4944 → c74c468`. Backend 423/423 (+85 от 338), frontend 252/252 (+16 от 236). 3 миграции (`20260512_program_types`, `20260513_program_templates`, `20260513_phase_stuck_alerts`), все идемпотентны. **Bug #12 закрыт полностью** (хардкод 'ПКС/acl' убран в HomeScreen, RoadmapScreen, telegramBot, services/api.js — multi-protocol foundation). **Bug #13 закрыт** (derived_title косметика + RehabProgramModal CreateWizard wizard). Архитектурные достижения: справочник `program_types` (3 типа), шаблоны программ (`program_templates` + junction), CreateWizard 3-step + EditForm 1-step (RehabProgramModal → директория), AdminContent 2 новых таба + 11 admin endpoints, instructor stuck-detection (yellow badge + opsAlert red push + weekly cron Mon 09:00 МСК). **Prod-smoke 4/5 ✅** 2026-05-15. **Сценарий 5 (red push)** — покрыт 8 mock-based unit-тестами, прод-проверка через плановый cron понедельник. **7 premise drift'ов** зафиксированы (TZ архитектора vs реальный код — формулы адаптированы). **8 backlog finding'ов** из smoke в memory. Полная сводка — [memory/wave_1_complete.md](.claude/projects/c--Users-------Desktop-Azarean-rehab/memory/wave_1_complete.md). Stacked PR strategy урок: PR base = предыдущая ветка, retarget на main + rebase auto-skip уже-merged коммитов через squash detection (no manual cherry-pick).
...
- OAuth Google/VK — TODO (применить тот же pattern: phone-match без password_hash filter + email-fallback + single-match anti-misroute)
- **OAuth post-registration autolink (2026-05-15, PR #62 `8974938`):** Bug #5 закрыт. Фильтр `password_hash IS NULL` убран из byPhone-query (заменён на `is_active = true` для отсечения deactivated). Local-registered пациент (через invite-flow с password_hash) теперь может залогиниться через Yandex/Telegram → autolink. `password_hash` **не затирается** при OAuth-логине — classical login продолжает работать. Yandex callback получил **email-fallback** (после phone-match): если phone не совпал, пробуем `LOWER(email) = $1 AND is_active = true` с single-match anti-misroute. Audit `OAUTH_AUTOLINK` теперь покрывает phone_autolink || email_autolink, в `details.link_type` для tracking. **Security-таблица** одобрена архитектором (см. PR #62 body). Telegram email-fallback пропущен (Telegram OIDC обычно не возвращает email). Backend 429 → 433 (+4 теста; test 5 семантика инвертирована).
...
| ~~13~~ | ~~MEDIUM~~ | ~~RehabProgramModal: «Комплекс #N»~~ → **ЗАКРЫТ ПОЛНОСТЬЮ 2026-05-15** Wave 1 коммитами 1.08a + 1.08b **+ PR #63 (root cause)**. **1.08a:** computed поле `derived_title` в 4 SELECT'ах `routes/complexes.js`. **1.08b:** новый shared `ComplexSelector.js` использует `c.derived_title || c.title || \`Комплекс #${c.id}\``. **PR #63 (`e016285`):** root cause — backend POST/PUT теперь принимают `title`, CreateComplex.js + EditComplex.js имеют input «Название комплекса» с pre-fill. Whitespace-only → NULL → derived_title fallback (backward compat). +4 теста, backend 433→437. |
...
| 15 | MEDIUM | **MDEditor (поле «Описание» в редакторе упражнения) — text color не подхватывает тему** — пакет `@uiw/react-md-editor` v4 автоимпортирует только `esm/index.css` с layout, а CSS-variables (`--color-fg-default`/`--color-canvas-default`) живут в `dist/mdeditor.css` которая не exposed через `package.json` `exports`. На светлой теме редактор пишет белым по белому, в тёмной непредсказуемо. Партнёрский баг к B6: глобальный `input/textarea/select` в `index.css` без background/color tokens — некоторые формы пациента/инструктора без локальных tokens могут показывать default browser white в тёмной теме. Тех-долг — заходить отдельным TZ от архитектора с дизайн-spec. Обнаружено и зафиксировано в prod-smoke 2026-05-12. **Подтверждено в prod-smoke 2026-05-15** — модалки AdminContent (Создать тип/фазу/шаблон/совет/видео) в Dark theme имеют невидимый input text. **AdminContent partial fix — 2026-05-16 PR #67** (commit `16ed04c`): добавлен `background: var(--color-surface-2)` в `.adminFormGroup input/select/textarea`. Остальные scope'ы Bug #15 (MDEditor + глобал в `index.css` + `color-scheme: dark` + другие формы) — открыты, ждут architect-spec. См. [memory/bug_admin_content_modal_dark_theme_text.md](.claude/projects/c--Users-------Desktop-Azarean-rehab/memory/bug_admin_content_modal_dark_theme_text.md). |
| 16 | MEDIUM | **Аватары пациентов теряются на каждом деплое** — [`.github/workflows/deploy.yml:181`](.github/workflows/deploy.yml#L181) делает `mv backend → backend.old.<ts>` и подкладывает новый каталог из release-tarball'а. В tarball'е `backend/uploads/` пустой (CI-клон), `--exclude='uploads'` в rsync на L120 нет → каждый push стирает аватары. Косметика `55a3205` отфильтровала 404 из ops-bot (фронт показывает initials), реальная причина открыта. Тот же риск над `backend/uploads/diary_photos/`. **Fix:** persistent volume `/var/lib/azarean-rehab/uploads` (создаётся раз в [setup.sh](deploy/setup.sh)) + симлинк `backend/uploads → /var/lib/...` после swap'а в deploy.yml + добавить uploads в [deploy/backup.sh](deploy/backup.sh). Подробности — [memory/bug_avatar_lost_on_deploy.md](.claude/projects/c--Users-------Desktop-Azarean-rehab/memory/bug_avatar_lost_on_deploy.md). |
