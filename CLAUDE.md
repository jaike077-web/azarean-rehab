...
- **Миграция** (образец `20260520_pain_character`): `VARCHAR(50)→TEXT[]` (USING: скаляр→1-эл.массив, NULL/''→NULL), btree→**GIN**-индекс, defensive CHECK (непустой массив без пустых элементов). **БЕЗ whitelist значений** — легаси из CSV-импорта мог занести свободный текст («Бедро»); строгий `<@ ARRAY[коды]` уронил бы конверсию на проде. Чистоту кодов держит app-слой (чекбоксы фронта + `mapArray` в структуризаторе). Идемпотентна (idem-цикл 88×2 чисто).
- **Backend:** `utils/bodyRegion.js toBodyRegionArray` (единая нормализация скаляр|массив→TEXT[], back-compat). `routes/exercises.js`: GET-фильтр `body_region @> $::text[]` (param=JS-массив) + stats `'код' = ANY(body_region)` + POST/PUT/bulk — **нативный pg-массив** (НЕ `JSON.stringify`, в отличие от equipment/position/rehab_phases-JSONB — РАЗНЫЕ семьи операторов!). `import.js` оборачивает CSV/Kinescope-строку в массив. `progress.js` `(body_region)[1]` — сохранён скалярный контракт мёртвого алиаса `exercise_category`. `exerciseStructuring.js` `mapScalar`→`mapArray` + промпт «массив, перечисляй все суставы» + few-shot присед=`["knee","hip"]`.
- **Frontend:** ExerciseModal «Регион тела» — **пилюли-тоглы** на всю ширину (1-я версия = чекбоксы-карточки в half-width формрядом → grid-сетка не раскладывалась в колонки → вертикальная «кишка»; фидбэк смоука Vadim «колхоз» → вынес на ширину + пилюли). Единый источник опций `BODY_REGIONS_OPTIONS` (убран локальный дубль). Порядок регионов везде (`exerciseConstants` + `exerciseStructuring`) — анатомически по группам: верх (плечо/локоть/кисть) → позвоночник → низ (ТБС/колено/голеностоп) → общее. Отображение (Card/View/Detail/Selector) + фильтры (Exercises/Selector/CreateComplex/EditTemplate) массив-aware (`formatBodyRegions`/`firstBodyRegion`/`bodyRegionMatches`), back-compat со скаляром. **Пациента НЕ трогает** (классификация библиотеки, не program_joint — проверено: `rehab.js`/PatientDashboard не используют body_region).
- **Доводка пилюль (PR #102 merge `7ed51f4`, SW v43→v44, CSS+JSX-only):** Оборудование/Положение/Фазы реабилитации в ExerciseModal тоже переведены с чекбоксов-карточек на пилюли → **вся секция «Параметры выполнения» единая**. CSS-классы `regionPill*`→общие `pill`/`pillGroup`/`pillActive` (старые `.checkboxGroup`/`.selectedTags`/`.tag` остались мёртвыми — чистка при случае). **Визуал каждой секции проверен скриншотом Playwright.** Деплой #102 пришёлся на пик параллельных мержей (#97–#101 от соседних чатов) — порядок сверен по `merge-base` (мой merge = ребёнок параллельных = суперсет, не клоббер), финал прода `a8248ec`/v44 с пилюлями. Урок закреплён: [[feedback_verify_merge_deploy_order]].
- **Гейты:** backend **1183**, frontend **764**, lint:modals, build (CI=false как деплой), HTTP round-trip через живой backend (create `["knee","hip"]`→фильтр→edit→null→stats), адверсариал-ревью (3 линзы/Workflow → **0 блокеров/major**, 5 nit, 1 починен). **Визуал проверен скриншотом через Playwright** (поймал «колхоз» сам, без ожидания юзера).
- 🛠️ **Playwright MCP установлен** (user-scope `~/.claude.json`, бэкап `.bak-premcp`) — скриншот-«глаза» на localhost для ЛЮБОГО проекта (активен после рестарта Claude Code; Windows: при `failed` в `/mcp` → `cmd /c npx`). Лекарство от «пишу CSS вслепую». [[infra-playwright-mcp-eyes]]. ⏭️ **СЛЕДУЮЩЕЕ:** этап-5 TTS / «доработать выбранное» в надиктовке; раздел гайда по упражнениям (мультирегион); расширение справочника (отделы позвоночника).

### 🟢 Надиктовка упражнений + цепочка качества (DeepSeek) — LIVE на проде (2026-06-24, SW v42)

**PR #94 (`88537fd`, SW v41) + PR #95 (`9cc0dde`, SW v42).** AI-конвейер наполнения библиотеки упражнений (НЕ-PII — контент библиотеки, не данные пациентов). Деплой надиктовки = cherry-pick 10 фича-коммитов с отставшей `docs/user-guide` на свежий main (НЕ мерж стейл-ветки; 1 конфликт `rehab.js` мультипрограмма vs variations — взял main + variations/progression в оба патиент-сборщика). Прод-секреты DeepSeek+SpeechKit залиты в прод-`.env` через SSH + PM2 рестарт + бэкап `.env.bak-presecrets`.

- **Пайплайн:** оператор диктует (🎙️ Yandex SpeechKit STT, raw PCM 16кГц) / печатает → **планировщик скрипта** (`POST /api/exercises/plan-script`, `deepseek-v4-pro` через `config.deepseek.modelPro`): из названия/черновика/надиктовки строит полный скрипт по чек-листу + `review_points` под вычитку эксперта (guard «Понял как: …» + при неоднозначном названии флаг «X, а не Y»; дозировки арифметически согласованы; «уточните» только в review_points, script = чистый пациент-текст) → **структуризатор** (`POST /api/exercises/structure`, `deepseek-v4-flash`, `services/structuringLlm.js` provider-agnostic, is*ai fallback): STT/текст → наши enum-поля, faithful (не выдумывает), форма «вы», глоссарий упрощения в description → **3 advisory-критика ПАРАЛЛЕЛЬНО** (`Promise.allSettled`): клин.sanity + №5 полнота + №6 стиль/обращение (синонимы НЕ флагает) → **оператор отвечает на пункты полноты/review_points** (поле ответа + «Вписать ответы в текст надиктовки» → его слова в текст → пересборка; AI вставляет ИМЕННО ответ, не выдумывает).
- **Инвариант:** источник клиники = эксперт, не LLM. faithfulness-ревьюер видит raw_text, ловит добавления/потери; автофикс ×1 → re-review. Стиль текстов — [[feedback_no_ai_slop_clinical_text]], [[feedback-faithful-not-clinical-reviewer]].
- **Вариации/Прогрессия** (миграция `20260619_exercise_variations_progression`, 2 nullable TEXT, аддитивно) — показ пациенту в ExerciseRunner (LOCKED-канон не тронут). `utils/videoEmbed.js` — единый парсер видео (Kinescope/YouTube/VK/Rutube/файл).
- **Метрики:** backend **1164**, frontend 758. Все критики обкатаны вживую Vadim. ✅ Мультивыбор `body_region` (скаляр→TEXT[]) — **сделан, LIVE** (см. блок выше). Точка входа: [[project-isai-integration]].

### 🟢 Визард создания программы Part A — LIVE на проде (2026-06-23, SW v40)

**PR #93 (merge в main).** Редизайн шага «1. Шаблон» (frontend-only): модалка шире только на шаге 1 (`.rehabProgramModal.wide` min(1080px,95vw) через `onStepChange`) + **табы зон** («Все»+program_joint, pure `getZones`) + клиентский **поиск** (pure `filterTemplates`, зона+query комбинируются) вместо вертикальной простыни. Part B (доказательная база) того же ТЗ — уже в проде (PR #90). [[project-wizard-template-picker-redesign]].

### 🟢 Плечевые протоколы «Путь» (9 протоколов, 3 волны) — LIVE на проде (2026-06-23)

**main = merge PR #87 (`eed1c42`), CI 3/3, прод отдаёт 9 типов** (проверены по одному из каждой волны). **Бэкенд-only → SW-бамп не нужен** (новые типы фронт-визард подхватывает динамически через `/program-templates`). **15 миграций** `20260619_shoulder_protocols_01..15` (имена с числовым инфиксом `_01_`..`_15_` → локаль-безопасны) + sanity-тест. Волна 1 (швы манжеты, _01..06): 4 RCR-типа (надостная/подостная/подлопаточная/массивный). Волна 2 (прочая хирургия, _07..10): декомпрессия + баллон. Волна 3 (консервативные, _11..15): субакр.синдром + кальцифицирующий тендинит + тендинопатия манжеты.
- **Метрики:** 9 program_types + 9 program_templates + **44 фазы** + **197 критериев**. Чисто аддитивно (только INSERT, acl/knee не тронуты → деплой без бэкапа). Полный бэкенд-сьют **1088/1088**. Браузер-смоук 9 карточек пройден Vadim.
- **Палитра:** хирургические = красный shield на ф1 (защита); консервативные = мягкая зелёная (ф1 = основная программа). Точка входа: [[project-shoulder-protocols]], `SESSION_HANDOFF_2026-06-19_SHOULDER_PROTOCOLS.md`.

### 🟢 Knee K1 мениск + доказательная база программ + полировка названий — LIVE на проде (2026-06-23, SW v37)

**main = merge PR #90 (`c3abe0e`) + хотфикс PR #91 (`18c9420`).** Три связанных блока:
- **K1 (протоколы мениска):** 5 program_types (`knee_meniscus_repair`/`root_repair`/`meniscectomy`/`conservative`/`allograft`, surgery T/T/T/F/T) + **24 фазы** + **139 критериев** + 5 шаблонов (миграции `20260622_knee_meniscus_01..07`). Аддитивно.
- **Part B (доказательная база):** `program_types.evidence_summary`/`evidence_sources` (аддитивный ALTER) + seed по **15 колено-протоколам** (10 LIVE + 5 мениск). Пациенту — summary в Markdown-подмножестве, инструктору — sources. Показ в 3 местах: карточка шаблона (тизер) + Step2 визарда + сноска внизу «Пути» (RoadmapScreen) + редактирование в AdminContent. Новый `frontend/src/components/RichText.js` — мини-рендерер Markdown без зависимостей (контент доверенный, не user input). Стиль текстов — живой профессиональный, якорь на реальные источники по имени ([[feedback_no_ai_slop_clinical_text]]).
- **Полировка названий:** `_zz_knee_labels_polish` — UPDATE label/title к корректной терминологии 15 колено (label=title; «Тендинопатия связки надколенника», «Реконструкция ПКС» и т.д.).
- ⚠️ **Грабля деплоя (хотфикс #91):** имена `..._evidence.sql` (ALTER) и `..._evidence_seed.sql` (UPDATE) — под UTF-8-локалью прода `sort` игнорирует пунктуацию → seed бежал ДО ALTER → `column does not exist` → деплой упал. В C-локали dev (`.`<`_`) не воспроизводилось. Фикс: ALTER → `..._evidence_col.sql` (`c`<`s`, стабильно в любой локали). [[dev-gotcha-migration-filename-locale-sort]]. Точка входа: [[project-wizard-template-picker-redesign]], `SESSION_HANDOFF_2026-06-22_KNEE_K1_EVIDENCE.md`.

### 🟢 Мультипрограмма M2.1 (связь зон) + цвет точки фазы — LIVE на проде (2026-06-23, SW v39)

Достройка мультипрограммного «Пути» (поверх M0+M1 ниже):
- **PR #85 (SW v38)** — `#3.2`: точка текущей фазы вторичной зоны «Пути» окрашена по цвету фазы (`getPhaseColor` в RoadmapScreen).
- **PR #86 (SW v39) + хотфикс PR #92** — **M2.1 связь зон:** `patients.zone_link_note` (аддитивный ALTER, миграция `20260619_patient_zone_link_note`) — инструктор-нота «как одна зона влияет на другую», показ карточкой вверху «Пути» при ≥2 зонах + поле в форме пациента. ⚠️ **Грабля мержа:** хенд-мёрж конфликта `patients.js` с уже-в-проде doctor_diagnosis (#88) — обе колонки в SELECT/INSERT/UPDATE, `zone_link_note` сдвинулась на `$8`; тест-ассерт позиции чинили хотфиксом #92 (CI Test-гейт отсёк деплой со старым тестом → прод не пострадал). [[project-multiprogram-path]].

### 🟢 Поле «диагноз от врача» (doctor_diagnosis) — LIVE на проде (2026-06-20, SW v36)

**PR #88 (merge `424ade5`) + SW-бамп PR #89 (`6f97a23`).** Справочное wellness-поле в карточке пациента (`patients.doctor_diagnosis`, миграция `20260620_patient_doctor_diagnosis.sql`). **Дисклеймер «со слов пациента / не мед.заключение» обязателен; это НЕ ЭМК, не расширять в медкарту без юр-проработки** ([[project_wellness_not_medical_positioning]]). Раздел гайда — `docs/guide/07-pacienty.md`. [[project-doctor-diagnosis-field]].

### 🟢 Мультипрограммный «Путь» (M0+M1) — LIVE на проде (2026-06-19, SW v35)

**main = merge `cc8bd93` (PR #83), CI run `27805360340` success, SW v35 LIVE** (curl прода: `azarean-v35` + `/api/health` ok, db alive — миграция `priority` применилась через migrate.sh). Пациент может иметь **несколько активных `rehab_programs` (зон)** с ранжированием: ведущая (`priority=1`) + вторичные по важности. Закрывает комбинированные кейсы (колено+ТБС, плечо+ТБС, +локоть). Коммиты: `8e6215a` (M0+M1) + `cc1c9ac` (полировка карточек зон). Метрики: backend **979**, frontend **727**, lint clean, build OK, миграция idempotent. Adversarial-ревью (3 линзы/24 агента, 1 баг исправлен). Браузер-смоук Vadim пройден.

- **M0 (фундамент):** миграция `20260614_rehab_program_priority` (`rehab_programs.priority SMALLINT` 1=ведущая + CHECK + partial-индекс, аддитивно/идемпотентно). `GET /my/dashboard` снял `LIMIT 1` → отдаёт `programs[]` (priority ASC, ведущая=[0], у каждой `.phase`); `program`/`phase` top-level сохранены (обратная совместимость). `POST /programs` принимает `priority` (или авто `MAX+1` подзапросом). `getStreakSummary` → **общий per-patient стрик** (DISTINCT `activity_date` по всем программам).
- **M1a (мультитрек «Путь»):** RoadmapScreen — 1 программа рендерится как раньше; ≥2 → карточки зон с **цвет-кодом региона** (knee→teal, shoulder→coral), ведущая = крупная заливная карточка + 2px контур + бейдж «Ведущая», вторичные = компактные с 2px контуром цвета региона + шеврон; future-expand по составному ключу `programId:phaseId`. HomeScreen индикатор «+N зон». **Грабля:** `.pd-rm button { border:none }` обнулял `border-style` у вторичных карточек (они `<button>`) → бордер задавать shorthand'ом, [[dev-gotcha-pd-rm-button-border-reset]].
- **M1b (упражнения по зонам):** `/my/exercises` → `resolveProgramExercises` на каждую активную программу; 1 → старая форма (blocks/legacy), ≥2 → `{mode:'multi-blocks', programs[], block_complex_ids:union}`. ExercisesScreen зоны с заголовками (сустав), `handleComplete` advance в правильный блок зоны.

> ⏭️ **M2.1 связь зон + цвет точки фазы — LIVE 2026-06-23** (см. блок выше). Отложено: **M2.2 оркестровка дня/цикла** при ≥2 комплексах (экран «Сегодня») — НЕ строить без явного запроса. **Хвосты TZ:** карточка-шапка при ОДНОЙ программе (сейчас голый таймлайн). Hardening гонки `priority` при параллельном создании — в M3. Детали: [[project-multiprogram-path]], [SESSION_HANDOFF_2026-06-19_MULTIPROGRAM_LIVE.md](SESSION_HANDOFF_2026-06-19_MULTIPROGRAM_LIVE.md).

### 🟢 Knee Protocols «Путь» + 10 шаблонов + полировка визарда — LIVE на проде (2026-06-03)

**Актуальный main = `2bc9cf9`** (поверх #80 → #79 `2d4def0` → #78 `dd845e3`), **SW v33 LIVE** (curl прода подтвердил). Три PR подряд приземлились в прод 2026-06-03, все CI 3/3 (Test/Build/Deploy):

- **PR #78 (`dd845e3`, CI `26885927727`)** — Knee Protocols «Путь»: дип-ресёрч по 10 патологиям колена → **10 program_types** (acl + 9 knee_*) + ~42 фазы + ~144 критерия в `rehab_phases`/`phase_transition_criteria` (seed-миграции `20260601_knee_protocols_01..11`). **acl refresh** не аддитивный (6→7 фаз, добавлена prehab-фаза 0; DELETE/INSERT критериев) — **бэкап прода `azarean_rehab-20260603-123452.sql.gz`** снят ДО деплоя (точка отката). Заодно влита `20260603_knee_program_templates_seed.sql` (`db27c19`) — **10 карточек program_templates** (`tpl_acl` + 9 `tpl_knee_*`) для шага «1. Шаблон» визарда, старый фаза-специфичный `acl_rehab` деактивирован (реверсивно). Детали: [[project-knee-pathology-phases]], `TZ_PATIENT_PATH_KNEE_PROTOCOLS.md`.
- **PR #79 (`2d4def0`, CI `26887819900`, SW v32)** — полировка визарда: (1) группировка карточек по суставу — все 10 колено под одним заголовком «Колено» (`groupTemplatesByJoint` по `program_joint`); (2) превью фаз протокола в Step2 («Что войдёт в программу: N фаз», эндпоинт `/program-templates/:id/phases` — раньше не был подключён к визарду). recommended_complex для колена пуст → честная пометка «назначается вручную».
- **PR #80 (`2bc9cf9`, CI `26897747491`, SW v33)** — «Текущая фаза» в Step2 = **динамический дропдаун** из реальных фаз протокола (ACL 1–6, knee_oa 1–3) вместо хардкода 1–4. `buildPhaseChoices` (pure): `phase_number>=1` → «N — title», fallback 1–6. Template-режим = фазы из превью #79; ручной = `getPhases(program_type)`. `current_phase` сброс на 1 при смене протокола.

Каждый PR: adversarial-ревью 3 линзы = ship, idempotency/тесты зелёные (фронт-сьют 56 suite/708, backend 957+). **Ключ:** `program_templates` (карточки шага 1) ≠ `program_types` (типы/протоколы + фазы «Пути», дропдаун «Тип программы»). Эндпоинт фаз — `?type=X` (query), НЕ `/phases/X`.

> ⏭️ **СЛЕДУЮЩЕЕ — D3 (prehab-as-start):** phase 0 (предоперационная) пока НЕ выбирается как стартовая — фронт (`handleCreate parseInt||1` + select onChange + `buildPhaseChoices` filter `>=1`), бэк (POST `/rehab/programs` [rehab.js:1509](backend/routes/rehab.js#L1509) `current_phase||1`, PUT :1577 `||null`) и RoadmapScreen ([:400](frontend/src/pages/PatientDashboard/components/RoadmapScreen.js#L400) `current_phase||1`) коэрсят 0→1. Полная поддержка = снять коэрсинг сквозь все 3 уровня. Точка входа — [SESSION_HANDOFF_2026-06-03_KNEE_WIZARD_D3.md](SESSION_HANDOFF_2026-06-03_KNEE_WIZARD_D3.md), [[project-d3-prehab-as-start]].

### 🟢 Exercise Audio (EA1–EA6) — LIVE на проде (2026-06-02)

**Актуальный main = `b9f9cdf`** (поверх EA-merge → CT-merge `c1aa3e8` → iPhone-доводки #73/#75/#76), **SW v29 LIVE** (curl прода подтвердил, `/api/health` ok). EA-арк приземлился merge `a97a41b` + date-flake fix `9e02126` поверх `914e4ca`, CI run `26776150464` — Test/Build/Deploy все success. Метрики на EA-merge (снимок): backend **52 suite / 922 tests**, frontend **53 suite / 653 tests**, lint:modals clean (133); CT-арк поднял до **957 / 678** (см. CT-блок ниже).

**EA = длинный звук (музыка / голос-инструкция / медитация), привязанный к УПРАЖНЕНИЮ** (отдельный персистентный слой от cue-бипов AA; играет ВЕСЬ период упражнения, бипы поверх, своя громкость). 6 чекпойнтов EA1→EA6, adversarial review на каждом:
- **EA1 миграция** `20260601_exercise_audio.sql` (аддитивно, идемпотентно): `audio_presets.kind` ('cue'\|'track', существующие→cue) + `exercises`(audio_preset_id FK→audio_presets SET NULL, audio_loop) = дефолт библиотеки + `complex_exercises`(audio_preset_id, audio_loop, audio_off) = перебить/выключить per-комплекс + 2 CHECK (`chk_ce_audio_off_no_preset`/`_no_loop`: off ⇒ нет preset/loop) + индексы на оба FK.
- **EA2 admin-backend:** `audioTrackUpload` (multer **10 МБ**, рядом с cue 512КБ) + `/api/admin/audio-presets?kind=track` (dispatch multer по `req.query.kind`, хранит/фильтрует kind). usage_count пресета считает 4 источника (cue defaults + complex cue + exercises + complex_exercises) → 409-on-delete.
- **EA3 привязка+резолв:** `utils/exerciseAudio.js` — SQL-фрагмент `RESOLVED_EXERCISE_AUDIO_SQL` + JOINs (ap_ce/ap_e) + `normalizeExerciseAudio` + `validateTrackPresetIds` (kind='track', **БЕЗ is_active** — неактивная привязка → резолв даёт нет звука, round-trip не падает 400). Резолв per-упражнение: `complex_exercises.audio_off` → complex override → library default → нет (только активный track; sig=preset.updated_at). exercises/complexes POST/PUT принимают audio; `/my-complexes/:id` И `/my/exercises` (blocks+legacy) отдают `audio:{preset_id,loop,sig}` у каждого упражнения; scoped serve `/audio-presets/:id/file` расширен на трек-привязки пациента.
- **EA4 admin UI:** ExerciseModal «Звук упражнения (трек)» select+цикл (всегда шлёт audio_preset_id → правка не-админом не затирает); per-row `ExerciseAudioControl` (наследовать/выкл/трек+цикл) в Create/EditComplex; AdminContent вкладка «Звуки» → суб-таб «Cue-сигналы | Треки упражнений» (`TrackPresetsTab`); cue-секция ↔ track разведены по kind. `validateTrackFile` (10МБ, без чека длительности).
- **EA5 раннер (iOS-критично):** AudioContext — персистентный music-плеер ОТДЕЛЬНЫМ `musicGain` (своя громкость `settings.musicVolume` 0.5, persist в `azarean_audio`). `startExerciseAudio` СТАБИЛЬНЫЙ (читает settingsRef → не рестартит при смене громкости), gate/dedup/gen-guard, **AudioBufferSourceNode ТОЛЬКО** (new Audio запрещён в раннере). ExerciseRunner (LOCKED) — **узкий unlock** (хук + 2 effect: старт на смене ce.audio + cleanup на анмаунте), канон не тронут (git diff verified). ProfileScreen слайдер «Громкость музыки».
- **EA6 GDPR:** no-op — трек-пресеты студийные admin-ассеты (не PII пациента), не в data-export / deletion-queue (там только `patient_audio_overrides` от AA6).

**Merge-грабля:** поверх параллельного `914e4ca` (audio ▶-tone-preview fix: `useAudioPreview(presetId, cueName)` синтезирует тон события) — конфликт `sw.js` (v24×2 → **v25**), AdminContent авто-смёрж (914e4ca preview-button + EA cuePresets filter сосуществуют). Мои one-arg track-превью совместимы с новой 2-arg сигнатурой (non-null preset → blob-путь). **Бонус:** закрыт преэкзистинг date-флак `DailyPainSection.test.js` (UTC toISOString → local today, иначе в ранне-утреннее окно Екб ломал CI Test-gate).

**✅ iPhone-смоук трека подтверждён Vadim на железе (2026-06-02):** музыка весь период work-фазы упражнения поверх cue, ползунок «Громкость музыки» в Профиле, смена/стоп на границах — работают. Три iPhone-доводки доведены и LIVE: **#73** трек играет в фазе work, не на ready-интро (`9f8bb8e`, SW v26); **#75** кнопка-мут 🔊/🔇 — мгновенная тишина трек+бипы через `settings.enabled` (`de1dec7`, SW v27); **#76** мут перенесён из шапки карточки в таймер-зону (timed: absolute угол кольца-зоны; rep-only: бар `.acts`; merge `b9f9cdf`, SW v29). Также закрыт хотфикс **#72** (`f278501`/merge `2aebcf7`): `POST/PUT /api/complexes` 500 при дефолтном (незаполненном) темпе — фронт слал `tempo_pause_s:null`, нормализатор без guard давал частичный tempo → нарушение CHECK `chk_ce_tempo`; фикс — guard `v===''||v==null → null` в обоих хелперах (backend-only, SW не бампали). AudioContext-unlock на реальном iOS локально/в CI не воспроизводится. Точка входа: [SESSION_HANDOFF_2026-06-01_EA_ARC_COMPLETE.md](SESSION_HANDOFF_2026-06-01_EA_ARC_COMPLETE.md), память [[project-exercise-audio]].

### 🟢 Custom Tone Editor (CT1–CT4) — LIVE на проде (2026-06-02)

**CT-merge = `c1aa3e8`** (CT1–CT4, поверх EA-merge `a5aac89`), **SW v28**; затем поверх лёг мут→таймер-зона `b9f9cdf` (SW v29 — актуальный main). Прод-verify: SW `azarean-v28`→`v29`, `/api/health` ok. Метрики на CT-merge: backend **957 tests**, frontend **678 tests**, lint:modals clean. **iPhone-смоук пройден** (Vadim 2026-06-02 «работает»).

**CT = админ редактирует параметры встроенного тона события (cue) — дом-карта (Вариант А, глобально на всю студию), per-комплекс НЕ делается** (complex_cue_sounds тон не несёт, только preset_id). Это AA-слой (cue-тон), параллельный EA-арку (длинный трек на упражнение). 4 чекпойнта, adversarial review на каждом:
- **CT1 миграция** `20260602_audio_cue_tone_config.sql` (аддитивно, идемпотентно, применена к dev): `audio_cue_defaults ADD COLUMN IF NOT EXISTS tone_config JSONB` (nullable). Форма: `{frequencies:[Гц 20..20000, 1..8 шт], durationMs: 20..2000, type ∈ sine/square/triangle/sawtooth}`. NULL → дефолт getCueConfig(cue). gain НЕ редактируем (всегда из getCueConfig). Валидация — application-layer (CT2), БД только nullable JSONB.
- **CT2 backend** (`20a8cec`): `PUT /api/admin/audio-cue-defaults/:cue` принимает опц. `tone_config` (CASE-preserve `$6::boolean` — lock/preset-toggle не затирают tone_config), GET отдаёт. Валидация `backend/utils/audioTone.js validateToneConfig` (frequencies 20..20000×1..8, durationMs 20..2000, type ∈ TONE_WAVE_TYPES, ровно 3 ключа). `patientAuth GET /my-complexes/:id` прикладывает `tone_config` из дом-карты в `audio_cues[]` когда resolved preset_id=null (тон глобален даже при per-комплекс явном тоне).
- **CT3 раннер** (`9497083`): `AudioContext.playTone` читает резолвнутый `tone_config` (programCuesRef) → перебивает frequencies/durationMs/type; gain из getCueConfig (base). Малформ → fallback на base (инвариант: не тишина если есть стандартный тон). iOS-инвариант (только OscillatorNode), ExerciseRunner (LOCKED) не тронут.
- **CT4 admin UI** (`fe50649`): AudioPresetsTab дом-карта — при preset_id=null суб-строка с 3 контролами (частота/длительность/форма волны) + «Сохранить тон». ▶-превью через `useAudioPreview.playStandardTone(cue, toneConfigOverride?)` — live-превью редактируемого тона. `frontend/src/utils/toneConfig.js` (pure, зеркало backend audioTone.js): `toneFormFromConfig`/`buildToneConfig` (clamp, пустое/NaN→дефолт, bad type→sine) ДО синтеза/сохранения. Адверс-ревью (3 линзы, 8 находок): 1 minor подтверждён (lock терял несохранённую правку → `load()` не чистит toneForms, `handleSaveTone` точечно сбрасывает свой cue), 7 refuted by-design.

**Коммиты:** CT1=`4a47254`, CT2=`20a8cec`, CT3=`9497083`, CT4=`fe50649`. SW bump=`6c7b759` (v27→v28). Merge=`c1aa3e8`. Память [[project-custom-tone-editor]], точка входа [SESSION_HANDOFF_2026-06-02_CUSTOM_TONE_EDITOR.md](SESSION_HANDOFF_2026-06-02_CUSTOM_TONE_EDITOR.md).

### 🟢 ARC-CYCLE микроцикл + Custom Audio — LIVE на проде (2026-06-01)

**На момент этого merge: HEAD = `0186817`** (merge ARC-CYCLE поверх audio-merge `a9ea058`), **SW v22** — исторический снимок (актуальный main `b9f9cdf`/SW v29, см. EA/CT-блоки выше). Две большие фичи влиты в прод подряд из двух параллельных сессий: **Custom Audio** (`a9ea058`, ветка `feature/custom-audio-upload`) + **ARC-CYCLE микроцикл** (`0186817`, ветка `feature/arc-cycle-microcycle`). Метрики на merge: backend **48 suite / 876 tests**, frontend **50 suite / 610 tests**, lint:modals clean (130).

**ARC-CYCLE = контейнер-слой над `rehab_programs.complex_id`** (одиночный FK). 7 чекпойнтов AC1→AC7 + smoke-fix'ы:
- **Миграция** `20260531_program_blocks.sql` — `program_blocks` (контейнер: `block_type ∈ {gymnastics, training}`, цель `target_*` + per-type unit, ротация `current_day_index`) + `program_block_complexes` (junction, `day_index` дни ротации). Аддитивно, БЕЗ backfill — legacy `complex_id` остаётся fallback'ом (пилотные пациенты без блоков работают как раньше).
- **Гимнастика** (плоский дневной набор, day-grained) vs **тренировка** (микроцикл День А/Б, ротация последовательностью, week-grained). Закрыл день → advance на следующий (по кругу), `POST /my/training/advance` + lazy-on-GET, идемпотентно через `last_advanced_session_id`.
- **Инструктор:** CRUD блоков `/api/rehab/programs/:id/blocks` + `/blocks/:blockId` (AC2); UI `BlockEditor` в RehabProgramModal (AC3).
- **Пациент D2-сплит** `/my/exercises` (AC4) → две секции (гимнастика ежедневно / тренировка текущий день) + `block_complex_ids` (будущие дни не протекают в «Другие комплексы»); ExercisesScreen hero-карточки (AC5 + визуал-полировка).
- **Командный центр — ДВА показателя адхеренса РАЗДЕЛЬНО** (Rule #34): `PER_PATIENT_STATE_SQL` пере-скоуплен с одного `ap.complex_id` на gymnastics-ось (`COUNT(DISTINCT completed_at::date)`, дневная дисциплина) + training-ось (`COUNT(DISTINCT session_id)`, anti-175%). C2 `adherence:{gymnastics:{adhering,no_target}, training:{...}}`, C3 per-instructor, C4 dynamics re-scope через `program_complex_ids`. EXISTS-гейт: пустой блок → no_target, не ложное «не соблюдает». FunnelPanel «Приверженность» (две оси) + InstructorModal две оси (AC6/AC7).

**Smoke-fix'ы (по итогам браузер-смоука):** (1) `/complexes/patient/:id` отдаёт ВСЕ комплексы пациента (не только текущего инструктора) + access-guard — фикс потери тренировочного дня при переназначении пациента (Wave 3); (2) `.blockRow min-width:0` — фикс распирания модалки длинным `<option>`; (3) D2 hero-карточки вместо плоских.

**Merge-граблы (две сессии, один main):** audio влилась первой (`a9ea058`), arc мёржилась поверх — 2 конфликта (sw.js версия → v22; complexes test describe). 4 файла авто-смёржились (admin.js audio AA + arc AC6, complexes.js, ExercisesScreen.js audio AA5 + arc D2, api.js). **Преэкзистинг флак (НЕ arc):** `patient_program_audio.routes.test.js` (AA3) использует общий реальный `uploads/sounds/presets` + другой audio-тест чистит `\d+.mp3` → редкая гонка параллельных jest-воркеров (~1 из 4 прогонов). Изолированно зелёный, продукт ОК. Бэклог аудио-сессии — уникальный temp-dir на тест-файл.

### 🟢 Runner таймер-зона = language-эталон (2026-05-29)

**main HEAD = `6151f66`**, **SW v18**, backend **729 tests**, frontend **40 suites / 510 tests**, lint:modals clean. Серия DA1 → DA2 → DA2.x → DA2.y → DA2.z поверх audio-арка `bb3f42f` (handoff 2026-05-29). Таймер-зона ExerciseRunner-карточки приведена к утверждённому language-эталону архитектора.

**Что сделано:**
- **DA1** (`ce9a6f9`, SW v14) — палитра таймер-зоны: ready-кольцо coral-track opacity .20 + БЕЗ progress-дуги, subdued teal-tint RestTimer-контролы (E1F5EE/9FE1CB/0F6E56) мельче без shadow, skip-rest расщеплён с finish (skip = solid teal forward, finish = клинически тихая outline coral), PhaseRing цифра tabular-nums.
- **DA2** (`d3e566a`, SW v15) — language-добивка: контекстная шапка (testid `phase-label`/`phase-context-label`: ready→«цель M:SS», countdown→«осталось», open-hold→«удержание», rest→«ОТДЫХ» + «до подхода N+1»), presets скрыты в auto-rest через `hidePresets` prop (рep-only ручной режим не задет), кнопки full-width (снят max-width:280), RestTimer ring 200→170 (унификация с PhaseRing).
- **DA2.x → DA2.y → DA2.z** (3 итерации gap, финал `6151f66`/SW v18) — **ROOT CAUSE найден через multi-agent workflow `w1dibjuew`** (6 explorers + synthesizer, high confidence): [PatientDashboard.css:2278](frontend/src/pages/PatientDashboard/PatientDashboard.css#L2278) содержит universal reset из LOCKED ExerciseRunner v4 iOS-эталона: `.pd-runner * { margin: 0; padding: 0; box-sizing: border-box }`. Specificity (0,1,0) равна `.pd-phase-actions`, но reset загружается ПОЗЖЕ в каскаде → побеждает. Все margin'ы 24→32→48 на skip/start/finish-кнопках и padding'и `.pd-phase-btn`/`.pd-rest-timer` зануливались до 0 — CSS правила формально на проде (curl-verified), но визуально не применялись. **Fix:** префикс `.pd-runner` на ВСЕ селекторы PhaseRing.css + RestTimer.css → specificity (0,2,0) → перебивает reset. См. правило [feedback_pd_runner_universal_reset.md](.claude/projects/c--Users-------Desktop-Azarean-rehab/memory/feedback_pd_runner_universal_reset.md).

**Lesson:** любое CSS правило с `margin`/`padding` внутри `.pd-runner` scope обязано иметь префикс `.pd-runner` (или другой класс) для bump specificity, иначе зануляется. Это **первая обнаруженная** инцидент-точка где LOCKED iOS-эталон скрытно влиял на post-CP CSS — bumping requirement стоит держать в голове при любых будущих стилях в раннер-карточке.

**Бэклог по аудио (открыто, не блокирует pilot):**
- Custom upload звуков (Vadim выбрал 2026-05-28, ~12-20ч, ждёт TZ архитектора с 6 decision points) — [backlog_custom_audio_upload.md](.claude/projects/c--Users-------Desktop-Azarean-rehab/memory/backlog_custom_audio_upload.md)
- CP3b темп-метроном (~4-6ч фронт-only, БД уже хранит tempo_eccentric/pause/concentric через CP2a)
- Возможный fix set_start vs set_end путаются на слух (на смоуке не подтверждено)
- Per-cue volume, БД-persist настроек, reps→time conversion

**Следующее:** custom audio (ждать TZ архитектора) или переход к экрану №2 (Главная / Путь / Дневник / Замеры / Связь) — кидать скрин что больше дизморалит, архитектор рисует таргет → TZ.

### 🟢 Wave 3 (Owner Command Center) — LIVE на проде + 2 hot-fix'а закрыты (2026-05-27)

**main HEAD = `6c47f35`** (после hot-fix PR #68 `a1a817b` + PR #69 поверх Wave 3 merge `db64dc8`), **tag `v0.2.0-command-center`** (на Wave 3 merge). Backend **706/37** (+5 от Wave 3 LIVE через bug #16 fix), frontend **393/33**, lint:modals clean. Pre-Wave-3: `3907034`. Wave 3 CI run `26457193975` — все 3 job'а success (Test 72s + Build 78s + Deploy 56s). Backend C1-C4 + frontend C5.1-C5.4 + миграция `20260526_instructor_assignment_and_cadence.sql` applied на проде 2026-05-26 в 15:17:56 UTC. **Pre-deploy backup:** `/opt/azarean-rehab/backups/azarean_rehab-20260526-151136.sql.gz` (60K). **Phase 7 verify ✅:** бэкфилл 2/2 пациентов, uploads symlink стоит (`backend/uploads → data/uploads`, внутри avatars/diary_photos/measurements), легаси-программ не-active = 0, PM2 `azarean-rehab` online. **Phase 8 browser smoke ✅:** 5/5 панелей живые, модалка инструктора с метриками + лента сигналов, тёмная тема, JARVIS не задет. SW bumped v7→v8.
...
**Hot-fix backlog после prod smoke — статус 2026-05-27:**
- ✅ **bug #16 ЗАКРЫТ (PR #68):** POST `/api/progress` 429 — был батч by-design (~15 POST + ~15 GET prevSession на workout) + слишком тесный глобальный лимитер 100/15min. Ветка 2a: `generalLimiter.skip('/progress')` + новый `progressLimiter` 600/15min/IP, prod-only mount на `/api/progress`. ExerciseRunner НЕ разблокирован, узкий scope. Прод-проверка ✅ — полный комплекс без 429. См. entry #72 в Завершённые исправления.
- ✅ **bug #17 ЗАКРЫТ (PR #69):** welcome screen на десктопе — `.pd-disclaimer-content` получил `max-width: 520px` + `margin: 0 auto`. Outer `.pd-disclaimer` остался fullscreen для background. Прод-проверка ✅. Entry #73 ниже.
- 🟡 **bug #18 (backlog, расширен 2026-05-27):** ExerciseRunner таймер без звука + **fresh feature scope** — пресеты звуков в настройках + **reps→time conversion** (20 повт ≈ 45 сек — пациентам легче по времени) + опц. ритмичный отсчёт. Уже не P3 bug, а Wave 3.x/4 feature. ExerciseRunner LOCKED, требует TZ архитектора + сверки эвристики reps→time с физиотерапевтом. См. [memory/feature_exercise_timer_no_sound.md](.claude/projects/c--Users-------Desktop-Azarean-rehab/memory/feature_exercise_timer_no_sound.md).
...
└── frontend/
    ...
    └── src/
        ...
        │   ├── useModalOverlayClose.js # close модалки только по чистому клику на overlay (CI lint gate)
        │   └── useUrlState.js          # screen/tab state в URL (?screen=/?tab=) — F5/назад-вперёд/ссылки (Вариант A, useSearchParams)
        ├── utils/
...
- **Пользовательский гайд `docs/guide/`** (README + 01–15, два трека: пациент 01–05 / оператор 06–15) — конечно-пользовательское руководство, отдельный слой «для людей» (CLAUDE.md остаётся источником правды для разработки). При заметной пользовательской фиче — обновлять затронутый раздел **в том же PR**: источник UI-подписей — реальный компонент `frontend/src/pages/...` (НЕ память, НЕ устаревшие `QUICK_REFERENCE.md`/`AGENTS.md`/`docs/PATIENTVIEW_AUDIT.md`); обнови дату в футере; **бамп Service Worker (`SW vN`) без правки `docs/guide/` = повод проверить гайд**. ТЗ и механизм актуальности: `TZ_USER_GUIDE_2026-06-04.md` (§9), чек-лист в `.github/pull_request_template.md`. Память: [[project-user-guide]].

### Гигиена веток и WIP (защита от потери работы)

> Поставлено после аудита 2026-06-14: в репо накопилось 58 dangling-коммитов + 5 заначек (раздел 6 ТЗ `TZ_GIT_HYGIENE_KIT`). Разбор — в `scripts/.lost-work-ack`.

- **Stash — карман на часы, не склад.** Уходишь от правки надолго → `git stash branch wip/<тема>` или коммит в ветку `wip/<тема>`. Заначка >24ч — находка детектора.
- **Ветку не удалять без merge-or-tag:** либо влита в `main`, либо `git tag archive/<имя> <ветка>` перед удалением. Особенно осторожно с активными параллельными ветками (`feature/multiprogram-path` и т.п. — там может идти авто-сессия).
- **Деплой = тег:** прод `my.azarean.ru` выкатывается push'ем в `main` (CI/CD `deploy.yml`). Каждый значимый выкат полезно метить `deploy/*` или `vX.Y.Z` — прод трассируется до коммита, задеплоенное не осиротеет.
- **Пуш рабочих веток — в тот же день** (`git push -u origin <ветка>`): локальный диск git'ом не бэкапится. CI/CD деплоит ТОЛЬКО `main` — пуш `wip/*`/`feature/*`/`chore/*` безопасен (деплоя не вызывает).
- **`scripts/check-lost-work.sh`** — детектор потерь (dangling + старые заначки + застрявшие/незапушенные ветки + грязное дерево). Гонять раз в неделю и перед сменой больших веток. Разобранную сироту вписывать в `scripts/.lost-work-ack` (full-sha + вердикт) — больше не всплывёт.
...
- **URL-state экранов/вкладок — через `useUrlState(key, default, opts)` hook** ([frontend/src/hooks/useUrlState.js](frontend/src/hooks/useUrlState.js)). Текущий экран пациента (`?screen=diary`) и вкладка инструктора (`?tab=patients`) живут в URL → F5 не сбрасывает экран, работают «назад/вперёд», на экран можно дать ссылку. Drop-in замена `useState` (Вариант A — родной `useSearchParams` react-router v7, URL = единственный источник правды, без useEffect/слушателей). НЕ возвращать `useState` для `screen`/`activeTab` и НЕ городить параллельный `?home=1`-костыль. `opts.valid` — guard от мусора и чужих ролей (инструктор не попадёт на admin-вкладку по прямой ссылке; `ProtectedRoute` отдаёт `user.role` уже на первом рендере). `opts` (valid/parse/serialize) держать на **модульном уровне** — стабильная ссылка, иначе сеттер пересоздаётся каждый рендер. Deep-link внутрь упражнения (`?ex=runner&complexId=&exerciseId=`) — отложено. PR #71 (`f45fefe`), SW v19. См. [[feature-url-state-routing]].
...
| ~~16~~ | ~~**CRITICAL**~~ | ~~POST /api/progress → 429~~ → **ЗАКРЫТ 2026-05-27** PR #68 Ветка 2a — не была петля, а батч by-design (workout = ~15 POST + ~15 GET prevSession ≈ 30-40 req) + слишком тесный `generalLimiter` 100/15min на весь /api. Серверный фикс: `skip('/progress')` в generalLimiter + новый `progressLimiter` 600/15min/IP prod-only mount на `/api/progress`. ExerciseRunner НЕ тронут. Прод-проверка ✅ (полный комплекс без 429). Backend 701→706 (+5 anti-regression). Entry #72 ниже. |
| ~~17~~ | ~~LOW~~ | ~~Welcome screen пациента на десктопе 100% ширины~~ → **ЗАКРЫТ 2026-05-27** PR #69 — `.pd-disclaimer-content` получил `max-width: 520px` + `margin: 0 auto`. Outer `.pd-disclaimer` остался fullscreen (inset:0) для background. Прод-проверка ✅ в incognito Chrome. Entry #73 ниже. |
| 18 | LOW | **ExerciseRunner таймер без звука** + **(расширено 2026-05-27)** выбор звуковых пресетов в настройках + **reps→time conversion** (пациентам легче делать по времени: 20 повт ≈ 45 сек — эвристика требует валидации физиотерапевтом) + опц. ритмичный отсчёт. Уже не P3 bug, а Wave 3.x/4 feature. ExerciseRunner LOCKED, требует TZ архитектора. См. [memory/feature_exercise_timer_no_sound.md](.claude/projects/c--Users-------Desktop-Azarean-rehab/memory/feature_exercise_timer_no_sound.md). |
...
### URL-state routing — экран/вкладка сохраняются в URL, LIVE (2026-05-29)
74. **URL-state routing (F5 + назад/вперёд + sharable ссылки):** PR #71 (`c40c731` → merge `f45fefe`), CI run `26647599499` — все 3 job'а success (Test + Build + Deploy to VDS). Перенос механики из JARVIS Director ([URL_ROUTING_GUIDE.md](URL_ROUTING_GUIDE.md)). Перед внедрением 2 multi-agent workflow: (a) верификация всех ~15 предпосылок гайда против реального кода, (b) адверсариальный ревью диффа (3 линзы → ship). Новый хук [frontend/src/hooks/useUrlState.js](frontend/src/hooks/useUrlState.js) — **Вариант A** (родной `useSearchParams` react-router v7, drop-in замена `useState`; URL = единственный источник правды, без useEffect/слушателей → нет циклов; стабильные дефолты parse/serialize на модульном уровне). **PatientDashboard:** `screen` (целое 0..5) ↔ `?screen=diary` через `SCREENS` slug-маппинг (switch не тронут, `SCREEN_URL_OPTS` на модульном уровне). **Dashboard:** `activeTab` ↔ `?tab=patients` + **гард по роли** (инструктор не попадёт на admin-вкладку по прямой ссылке — `ProtectedRoute` отдаёт `user.role` на первом рендере, без «мигания»); убран костыль `?home=1` (+ `useLocation`); `BASE_TABS/ADMIN_TABS/ALL_TABS` на модульном уровне. **Patients.js:** «Главная» `?home=1`→`?tab=home` (заодно починило `/dashboard?tab=patients` из PatientProgress, которое раньше не срабатывало). **ExerciseRunner (LOCKED) не тронут** — рендерится как child, view-состояние в ExercisesScreen. Тесты: новый [useUrlState.test.js](frontend/src/hooks/useUrlState.test.js) (6, против faithful useSearchParams через локальный mock), stateful `useSearchParams` добавлен в [__mocks__/react-router-dom.js](frontend/src/__mocks__/react-router-dom.js). Frontend 41 suite / 516 tests, lint:modals clean, build OK. SW bump v18→v19. Browser smoke A1-A7 + B1-B6 — зелёные. **Отложено:** 5.3 deep-link внутрь упражнения (`?ex=runner&complexId=&exerciseId=`). **Грабля jest-мока** (CRA roots=src авто-применяет мок глобально; `requireActual` на react-router v7 падает): [memory/dev_gotcha_react_router_mock_global.md](.claude/projects/c--Users-------Desktop-Azarean-rehab/memory/dev_gotcha_react_router_mock_global.md). См. [memory/feature_url_state_routing.md](.claude/projects/c--Users-------Desktop-Azarean-rehab/memory/feature_url_state_routing.md).

### Post-Wave-3 hot-fixes — 2 prod-smoke бага закрыты (2026-05-27)
73. **bug #17 — welcome screen 100% width на десктопе:** PR #69 (`4cb52b8` → merge `6c47f35`), CI 3:39 (Test 71s + Build 80s + Deploy 68s). Pre-existing P3 cosmetic с момента создания PatientAuth (Спринт 0.1). `.pd-disclaimer-content` был `flex: 1` без ограничения по ширине → на десктопе 1920+ тянулся на весь viewport. **Fix (1 файл, +7 строк):** [frontend/src/pages/PatientDashboard/PatientDashboard.css:195-208](frontend/src/pages/PatientDashboard/PatientDashboard.css#L195) — добавлены `max-width: 520px; width: 100%; margin-left/right: auto`. Outer `.pd-disclaimer` остался fullscreen (`inset: 0`) — критично для background-color, который закрывает остальной dashboard сзади. На мобиле <520px max-width не срабатывает, поведение не меняется. Тесты frontend 393/33 без изменений (CSS-only, jest не проверяет визуал, PatientDashboard suite 17/17 passed). lint:modals clean. Прод-проверка ✅ в incognito Chrome — welcome центрирован 520px-контейнером. См. [memory/bug_patient_welcome_desktop_layout.md](.claude/projects/c--Users-------Desktop-Azarean-rehab/memory/bug_patient_welcome_desktop_layout.md).

72. **bug #16 — POST /api/progress 429 loop (CRITICAL блокер пилота):** PR #68 Ветка 2a (`67529b6` → merge `a1a817b`), CI 3:19 (Test 73s + Build 76s + Deploy 50s). **Не была петля.** Part A диагностика дала 6 анти-петельных проверок отрицательные: единственный call site `ExerciseRunner.js:105` защищён `saving`/`disabled`, useEffect'ы делают только GET, interceptor 429 не ретраит, SW non-GET пропускает, лимитер на правильном уровне. **Реальная причина** — батч by-design + слишком тесный глобальный лимитер: один workout = ~15 POST `/progress` + ~15 GET `/progress/exercise/:id/complex/:id` (prevSession useEffect), 3-4 тренировки за окно 15 мин выжигали `generalLimiter` 100/15min. **Серверный фикс** ([backend/server.js:81-122](backend/server.js#L81)): `generalLimiter` получил `skip: (req) => req.path.startsWith('/progress')` (под `app.use('/api', ...)` Express strip'ит /api префикс, поэтому именно `/progress`). Новый `progressLimiter = rateLimit({ windowMs: 15min, max: prod ? 600 : 1000 })`, prod-only mount `app.use('/api/progress', progressLimiter)` — один mount покрывает И POST, И GET prevSession. 600/15min ≈ 15-20 полных тренировок (с запасом для пилота, abuse-защита остаётся). **Anti-regression** ([ExercisesScreen.test.js:174](frontend/src/pages/PatientDashboard/components/ExercisesScreen.test.js#L174)): добавлен `expect(progressPatient.create).toHaveBeenCalledTimes(1)` рядом с `toHaveBeenCalledWith` — программный эквивалент A1 для петли (зелёный → один клик = один POST, красный → эскалация на Ветку 1 с ref-guard в ExerciseRunner). **Новый backend sanity** `backend/tests/__tests__/progress_rate_limit.test.js` (5 file-content проверок: skip predicate, progressLimiter 600/15min, mount /api/progress, prod-only блок, разные лимитеры — защита от рефакторинга-объединения). Backend 701/36 → 706/37 (+5 тестов, +1 suite), frontend 393/33 без изменений, lint:modals clean. **ExerciseRunner НЕ разблокирован, НЕ тронут** (LOCKED, узкий scope). `type="button"` на done/skip кнопках — мина в backlog (не root cause). Прод-проверка ✅ — полный комплекс пройден без 429 (incognito пациент id=12). См. [memory/bug_progress_429_loop.md](.claude/projects/c--Users-------Desktop-Azarean-rehab/memory/bug_progress_429_loop.md). **Урок:** глобальный `generalLimiter` на весь `/api/*` слишком груб для приложения где **один user action может легитимно стрелять N API-вызовами**. Endpoint'ы под burst-нагрузку (`/progress`, future batch flows) требуют отдельных щедрых лимитеров через `skip` predicate + `app.use('/api/X', specificLimiter)`.
